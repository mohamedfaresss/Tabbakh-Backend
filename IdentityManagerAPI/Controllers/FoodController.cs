...
namespace Graduation_project_APIs.Controllers
{
    ...
    [HttpGet("ingredients/search/by-name/{ingredientNames}")]
    public IActionResult SearchIngredientIdsByName(string ingredientNames)
    {
        var names = ingredientNames.Split('.', StringSplitOptions.RemoveEmptyEntries)
        ...
        if (IsArabicRequest())
        {
            var ingredients = _arabicDb.المكونات
                .AsNoTracking()
                .ToList()
                .Where(ing => names.Any(name => ing.اسم_المكون != null && ing.اسم_المكون.ToLower().Contains(name)))
        ...
        else
        {
            var ingredients = _db.Ingredient
                .AsNoTracking()
                .ToList()
                .Where(ing => names.Any(name => ing.Ingredient_Name != null && ing.Ingredient_Name.ToLower().Contains(name)))
